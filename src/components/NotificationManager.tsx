...
import { useApp, DayKey } from '../app/AppContext';
...
function parseNotificationTag(rawTag: string) {
  const tag = rawTag.replace(/-snooze$/, '');
  const parts = tag.split('-');
  if (parts.length < 3) return null;

  const prefixed = parts[0] === 'fg' || parts[0] === 'alarm' || parts[0] === 'snooze';
  const start = prefixed ? 1 : 0;
  const medicineId = parts[start];
  const scheduleTime = parts[start + 1];
  const date = parts.slice(start + 2).join('-');
...
export default function NotificationManager() {
  ...
  useEffect(() => {
    ...
    const handler = (event: MessageEvent) => {
      ...
      const parsed = parseNotificationTag(tag);
      if (!parsed) return;
  ...
  }, [user, medicines, dailyLogs, markTaken, markSkipped, markSnoozed]);
